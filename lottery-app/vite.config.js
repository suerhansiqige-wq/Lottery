        ...
        if (req.method === 'POST' && req.url === '/api/save-draws') {
          ...
          req.on('end', () => {
            try {
              ...
              // 过滤新数据
              const newDraws = draws.filter(d => !existingIssues.has(d.issue))
    ...
    proxy: {
      '/api/lottery': {
        ...
        rewrite: (path) => {
          ...
          const count = url.searchParams.get('issueCount') || '30';
          const name = url.searchParams.get('name') || '3d';
          // 乐彩网数据文件：3d_desc.txt(福彩3D倒序), pl32_desc.txt(排列三倒序)
          if (name === 'pl3') return `/pl32_desc.txt`;
          return `/3d_desc.txt`;
        },
        headers: {
